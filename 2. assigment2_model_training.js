...
using CSV, DataFrames,MLJ, Random, MLJLinearModels, MLJDecisionTreeInterface, Plots,DecisionTree,PyPlot

# ╔═╡ 4e390ac0-5ea5-11ee-0143-d1e4d22000b0
md"""
...
begin
	df_ds2_switzerland_clean = CSV.File("./cleaned_data/ds2_switzerland_clean.csv",header=true, delim=',') |> DataFrame
...
md"Below is the swizerland output type"

# ╔═╡ b5c57add-2722-4f2e-afdb-4a40247d8f59
unique(df_ds2_switzerland_clean.num)

# ╔═╡ 6af154e9-1d1d-447d-b154-033539127569
md"Below is the hungarian output type"

# ╔═╡ ef8202b8-ac24-4ed2-8b5f-1fbc1db560df
unique(df_ds2_hungarian_clean.num)

# ╔═╡ 9a441c28-2b03-4152-9a57-7fda7e9aa0f8
md"Below is the va output type"

# ╔═╡ 04914f77-08dd-4b9a-aabb-f158bed7f2a5
unique(df_ds2_va_clean.num)

# ╔═╡ 02259867-0ccc-4302-8fba-b92420f34208
md"Based on above distinct output datatype, we will used to definne number of trees or classification per different datasets"

# ╔═╡ f969b3c9-1e1b-4c9a-951c-2a1f4eb28744
begin
	y= Integer.(df_ds2_switzerland_clean.num)
	X = dropmissing(select(df_ds2_switzerland_clean,Not(:num)))
	tree_reg = DecisionTreeRegressor()
	forest = EnsembleModel(model=tree_reg, n=5) #switzerland only has 5 type of outputs
end
...
begin
	Random.seed!(1)
	r_max_depth = range(forest, :(model.max_depth), lower=2, upper=20)
	r_subfeature = range(forest, :(model.n_subfeatures), lower=1, upper=10)
	tm = TunedModel(forest, tuning=Grid(resolution=10),
	                resampling=CV(nfolds=10), ranges=[r_max_depth, r_subfeature],
	                measure=rms)
 	mach = machine(tm, X, y)
	MLJ.fit!(mach);
end
...
begin
	rep = report(mach)
	res = rep.plotting
	vals_max_depth = res.parameter_values[:, 1]
	vals_n_subfeatures = res.parameter_values[:, 2]
end

# ╔═╡ 2d1a0243-b4cd-491b-b4bd-89b0daab4c9a
res
...
begin
	...
	tricontourf(vals_max_depth, vals_n_subfeatures, res.measurements, cmap=get_cmap("hot"))
	xlabel("#Max Depth", fontsize=14)
	ylabel("# of Subfeatures", fontsize=14)
	colorbar(label="RMS")
	p
end
...
begin
	...
	decision_tree = DecisionTreeRegressor(max_depth = 7, min_samples_leaf = 5, min_samples_split = 2, min_purity_increase = 0.0, n_subfeatures = 9, post_prune = false, merge_purity_threshold = 1.0, feature_importance = :impurity)
...
y_pred= round.(MLJ.predict(m, rows=test))
...
function find_best_RF_model(df,n_trees)
		...
		forest = EnsembleModel(model=model, n=n_trees)
		Random.seed!(1)
		r_max_depth = range(forest, :(model.max_depth), lower=2, upper=20)
		r_subfeature = range(forest, :(model.n_subfeatures), lower=1, upper=10)
		tm = TunedModel(forest, tuning=Grid(resolution=10),
	 	...
	 	mach = machine(tm, X, y)
		MLJ.fit!(mach)
		return report(mach).best_history_entry.model, X, y, train, test
end

# ╔═╡ ed889ac7-d296-4f3e-8b22-743ac9c2802d
model_hug,X_hug, y_hug, train_hug, test_hug = find_best_RF_model(df_ds2_hungarian_clean, 2); #hungarian dataset only has 2 types of output data
...
md"Let's build the regression model based on the output"

# ╔═╡ 9b79699a-b042-4c1e-888d-836a6be62726
begin	
	decision_tree_hug = DecisionTreeRegressor(max_depth = 4, min_samples_leaf = 5, min_samples_split = 2, min_purity_increase = 0.0, n_subfeatures = 4, post_prune = false, merge_purity_threshold = 1.0)
	
	m_hug = machine(decision_tree_hug, X_hug, y_hug)
	MLJ.fit!(m_hug, rows=train_hug, verbosity=0)
	y_pred_hug = round.(MLJ.predict(m_hug, rows=test_hug))
	accuracy(mode.(y_pred_hug),y_hug[test_hug])
end
...
md"Let's build the regression model based on the output for va dataset"

# ╔═╡ 4f33b0d4-a5ac-4021-8e94-2b0cd70f4030
begin	
	decision_tree_va = DecisionTreeRegressor(max_depth = 4, min_samples_leaf = 5, min_samples_split = 2, min_purity_increase = 0.0, n_subfeatures = 4, post_prune = false, merge_purity_threshold = 1.0)
	
	m_va = machine(decision_tree_va, X_va, y_va)
	MLJ.fit!(m_va, rows=train_va, verbosity=0)
	y_pred_va = round.(MLJ.predict(m_va, rows=test_va))
	accuracy(mode.(y_pred_va),y_va[test_va])
end
...
md"""Training Accuracy: The steady climb and eventual stabilization of the training accuracy at 1.00 for tree depths beyond 8 confirms that as the decision tree becomes deeper (more complex), it is able to fit the training data almost perfectly. This behavior is consistent with the nature of decision trees, as they can create complex boundaries to separate the data. However, perfect accuracy on training data can be an indication of overfitting.

Testing Accuracy Dips: The testing accuracy dips to its lowest point at a tree depth of 4, suggesting that a tree of this depth may be too simple to capture the patterns in the data. However, the subsequent increase in testing accuracy as the tree depth grows to 7 indicates that a more complex tree improves performance on unseen data up to this point.

Overfitting Indications: The peak in testing accuracy at a tree depth of around 7, followed by a slight dip as the depth increases further, is an early indication of overfitting. While the decline is not steep, it suggests that the additional complexity added by deeper trees (beyond depth 7) isn't contributing positively to performance on unseen data.

Optimal Depth: From the plot description, a tree depth of around 7 seems optimal. It offers a good balance between model complexity and performance on unseen data. Beyond this depth, there are diminishing returns in terms of testing accuracy.

Recommendations:

Model Selection: Consider using a tree with a depth of 7 for this dataset, as it seems to offer the best testing accuracy without introducing too much complexity.
...
# ╔═╡ 44e9be0c-784d-46de-b9a7-10ea603c6dea
md"""
### CROSS VALIDATION
"""

# ╔═╡ a4edee83-f60c-4f6f-b071-56267b3c07fb
# function n_fold_cross_validation(model, X, y, measure, nfolds=10,repeats=1)
# 	cv = CV(nfolds=nfolds)
# 	results= evaluate(model, X, y,resampling=cv, measure=measure, verbosity=0, repeats=repeats)
# 	return results
# end

# ╔═╡ e2d46bd6-0b0f-40e9-8e96-0b1c72159e0a
function n_fold_cross_validation(model, X, y, measure, nfolds=10, repeats=1)
    cv = CV(nfolds=nfolds)
    results = evaluate(model, X, y, resampling=cv, measure=measure, verbosity=0, repeats=repeats)
    return results
...
# ╔═╡ c3317247-8c14-4cbf-ab8b-dbfc89584926
dt_cv = n_fold_cross_validation(decision_tree, X, y, rms, 10)

# ╔═╡ 4a6b8ad4-ab34-469c-9b6e-7c084db424b5
begin
    using StatsPlots
    violin(["Random Forerest"], dt_cv.per_fold[1], label=nothing)
end

# ╔═╡ e3d4e687-7858-4dfe-8b28-8d9c4e83b47d
function draw_accuracy_curve(X,y)	
	t1accu_tr = []
	t1accu_te = []
...
draw_accuracy_curve(X,y)

# ╔═╡ bd1d3612-e21b-4460-8512-7dfc2c6c49cc
function find_best_DT_model(df,n_trees)
	train_arr = []
    test_arr = []
	Random.seed!(1)
	y = categorical(df.num)
	X = dropmissing(select(df,Not(:num)))
	train, testing = partition(1:nrows(X), 0.7, shuffle=true)

    for depth in n_trees
        tree_T1 = DTree(max_depth=depth)
        mach_T1 = machine(tree_T1, X, y)
        MLJ.fit!(mach_T1, rows=train, verbosity=0)
        push!(train_arr, accuracy(predict_mode(mach_T1, rows=train), y[train]))
        push!(test_arr, accuracy(predict_mode(mach_T1, rows=test), y[test]))
    end
    
    Plots.plot(n_trees, train_arr, label="Training")
    Plots.plot!(n_trees, test_arr, label="Tesing")
end

# ╔═╡ a8849990-291d-4ade-9e11-356f82f380a7
find_best_DT_model(df_ds2_hungarian_clean,1:15)

# ╔═╡ 5b87a994-8d58-413c-8994-26475b053110
find_best_DT_model(df_ds2_switzerland_clean,1:15)

# ╔═╡ 81f9c5d3-61c7-4426-b2b4-36cdba1c548a
find_best_DT_model(df_ds2_va_clean,1:15)

# ╔═╡ 998eb282-b420-4c7d-adce-cbe1018b07eb
dt_cv.per_fold[1]
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "707c92af56855836fb5a14b39142eea1a27eabbd"
