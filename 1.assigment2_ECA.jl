...
# ╔═╡ 31cd14cd-1ef3-4489-85b0-184650e73da1
using CSV, DataFrames,Plots,Statistics,PairPlots,StatsPlots,BetaML

# ╔═╡ 07fcd116-7cbb-452e-a4a1-aa87c80446f4
...
7. **restecg**: resting electrocardiographi results
   - Value 0: normal
...
**DS1 data is loaded into dataframe and preview is below:**
"""

# ╔═╡ 334e24f6-8a21-47d4-8904-639d763e8a3e
begin
...
# ╔═╡ 43692c3b-b4ab-48b6-9aab-d236a655d8fa
md"Now, we convert the dataframe into matrix and transpose the data so that it is Julia-styled, one data instance per column:"

# ╔═╡ bf0bc4b3-5ac0-4a06-9908-999fc1e6e4eb
begin
	ds1 = Matrix(df_ds1)
	X_ds1 = ds1'
end

# ╔═╡ 472b0124-42fc-4226-bb92-10c5fcd8ee81
md"""
Below is the distribution of variouse attributes. As we can see, **age**, **trestbps**(resting blood pressure), **chol**(serum cholestoral), **thalach**(maximum heart rate achieved) and **oldpeak** attributes have curved belt distribution. Especially, **age** follows normal distribution, **trestbps**, **chol** and **oldpeak** have right skewed distribution. **thalach** follows left skewed distribution.

Rest attributes, which are categorical date, hence they don't follow curved belt distribution.
"""

# ╔═╡ 871093e3-bfeb-4a88-9ce5-a607ca1c7eed
begin
	plots=[]
	for i in 1:14
		title = "Distribution Histogram for $(names(df_ds1)[i])"
		p = histogram(X_ds1[i,:], title = title, label="", titlefont=8)
		push!(plots, p)
	end
	Plots.plot(plots..., layout=(7, 2), size=(800,1200))
end
...
# ╔═╡ 2fefb85f-6301-457e-9f7e-7de91bb9a525
md"Now, we just need to make sure to convert categorical columns to integer, and rest columns leave as Float"

# ╔═╡ 220e7285-2d64-409c-b74b-f7c8633acc82
begin
...
# ╔═╡ a0875c5d-502f-490b-b635-c5deedfbf771
df_ds1
...
# ╔═╡ d3933aa3-8844-4f16-92b3-ae742f938af1
# pairplot(df_ds1)
begin
	sel_col_lst = [1,4,5,8,10]
	gr(size=(900,900))
	corrplot(ds1[:,sel_col_lst], label = names(df_ds1)[[1,4,5,8,10]])
end

# ╔═╡ 69e37d9e-1366-49ff-adbc-838a01f6be19
begin
	df_ds1_corr = hcat(DataFrame(Attribute = ["age","trestbps","chol","thalach","oldpeak"]),DataFrame(round.(cor(ds1[:,sel_col_lst]), digits=3),["age","trestbps","chol","thalach","oldpeak"]))
end

# ╔═╡ 4fd86e82-80e0-4a66-ada2-19ca293b4bad
md"Now, let's visualise the matrix correlation table by creating a heatmap."

# ╔═╡ ce24d31c-0f6f-4ff5-ba0b-fc2722f6fd4b
function heatmap_cor(df)
    ...
    heatmap(cm, 
        ...
        yticks = (1:m,cols),
        yflip=true)
    annotate!([(j, i, StatsPlots.text(round(cm[i,j],digits=3),
                       8,"Computer Modern",:black))
           for i in 1:n for j in 1:m])

end

# ╔═╡ ea5bdaf2-c21d-4349-8d9e-14ca7ff154d8
names(df_ds1)[[1,4,5,8,10]]

# ╔═╡ 9267aa4c-df76-4956-884c-bda176090819
heatmap_cor(df_ds1[:,[1,4,5,8,10]])

# ╔═╡ 37ade17e-ebd3-45d8-b5f3-c2c64adcb521
md"""
As we can see from the matrix table above, there is no strong correlation amount those 5 factors. The highest correlation value is -0.402 between age and thalach(maximum heart rate achieved), but this is still not a strong correlation value.
thalach and oldpeak has relatively high negative correlation as well.
"""

# ╔═╡ 720ae0f5-e480-4ed8-ba65-f33653652be6
md"### **TASK 2** - Data imputation"

# ╔═╡ dbe48956-a877-44d2-9ee5-113c800f776d
md"""
Impute at following three datasets (the processed.location.data files)
from DS2:
- processed.hungarian
...
begin
	column_dtypes = eltype.(eachcol(df_ds1))
	column_names = names(df_ds1)
	column_types = Dict(zip(column_names, column_dtypes))
end

# ╔═╡ 4330d8d9-06eb-4bf9-9d6e-f088266481f2
df_ds2_hungarian = CSV.read("./data/ds2_heart_disease/processed.hungarian.data",header=header,
	types=column_types,
	DataFrame, 
...
# ╔═╡ 89d7fd58-9e61-41d5-ad01-8378e45c1d62
df_ds2_switzerland = CSV.read("./data/ds2_heart_disease/processed.switzerland.data",header=header, 
    types=column_types,
	DataFrame, 
	#unconverted data will be igored and changed to "missing"
	strict=false, 
...
df_ds2_va = CSV.read("./data/ds2_heart_disease/processed.va.data",header=header, 
    types=column_types,
	DataFrame, 
	#unconverted data will be igored and changed to "missing"
	strict=false, 
...
begin
	df_list = [df_ds2_hungarian,df_ds2_switzerland,df_ds2_va]
	good_records = []
	for df in df_list
		push!(good_records, size(Impute.filter(df; dims=:rows))[1])
	end
	good_records
end
...
md"""
As we can see only $("df_ds2_hungarian") and $("df_ds2_va") dataframe has only one valid value. Obviously can not just simply remove data with missing value.

Let's look deeper into one dataset. In this case, lets's have look at all datasets's columns that have missing values.
"""

# ╔═╡ 93cb65e4-8345-4682-ba7b-bde12c376511
begin
	df_names = ["df_ds2_hungarian","df_ds2_switzerland","df_ds2_va"]
	missing_list = []
	for i in 1:3 
		df_missing = filter(d-> d.nmissing > 0, describe(df_list[i]))
		df_missing.total_rows = fill(nrow(df_list[i]), nrow(df_missing))
		df_missing.missing_ratio = round.(df_missing.nmissing/nrow(df_list[i])*100,digits=2) 
		df_missing.data_source = fill(df_names[i], nrow(df_missing))
		push!(missing_list,select!(df_missing,[:data_source,:variable,:nmissing,:total_rows,:missing_ratio]))
	end
	df_missing=sort!(vcat(missing_list[1],missing_list[2],missing_list[3]),[:missing_ratio], rev=true)
end
...
md"""To overcome the missing value issue, we use Hyperparameters for RandomForestEstimator (Random Forest) method in during the imputation process."""

# ╔═╡ 31d65c24-005b-4f85-b1c8-c12ac0df4ff0
# Function to perform Random Forest imputation on a dataset using RFImputer with RFImputerHyperParametersSet
function impute_with_rf(dataset::DataFrame)
	# Convert the DataFrame into a matrix
	data_matrix = hcat([convert(Vector, dataset[!, col]) for col in names(dataset)]...)

	# Define the hyperparameters for the RFImputer's underlying RF
	rfhpar = RFHyperParametersSet(
		n_trees = 20,
		max_depth = 10
		# Add other RF-specific hyperparameters here
	)

	hyperparameters = RFImputerHyperParametersSet(
		rfhpar=rfhpar,
	...
	# Create the default options for RFImputer
	default_options = BetaMLDefaultOptionsSet()

	# Create and fit the RFImputer model
	model = RFImputer(hyperparameters, default_options, nothing, nothing, false, Dict{String, Any}())
	imputed_data = BetaML.fit!(model, data_matrix)  # Note: Using BetaML.fit! as mentioned

	# Convert the imputed matrix back into a DataFrame
	imputed_df = DataFrame(imputed_data, names(dataset))
...
begin
	cleaned_df_list=[]
		for df in df_list
			df1 = impute_with_rf(df)
			# drop ca column in each dataset
...
begin
	df_ds2_hungarian_clean = cleaned_df_list[1]
	df_ds2_switzerland_clean = cleaned_df_list[2]
	df_ds2_va_clean = cleaned_df_list[3];
end

# ╔═╡ a85eb71b-df02-4c08-bd24-6e8b3a1d8ac7
for df in [df_ds2_hungarian_clean,df_ds2_switzerland_clean,df_ds2_va_clean]
	begin
		for i in 1: size(df)[2]
		    if !(i in numerical_columns)
				df[!,i] = Int.(round.(df[:,i]))
				println("Change column $(names(df)[i]) to Integr!")
			else
				df[!,i] = Float64.(df[:,i])
				println("Change column $(names(df)[i]) to Float!")
			end
		end
	end
end
...
select!(describe(df_ds2_hungarian_clean), Not(:eltype))

# ╔═╡ 395cd0d9-c9bb-48e0-953e-872c569e5e8d
md"**Switzerland location data**"

# ╔═╡ fc6ed8d3-eff5-41ff-b8f4-8c8ad443fd1e
select!(describe(df_ds2_switzerland_clean), Not(:eltype))
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "f76041eea539c540a78d72b1599786c153c8afc8"
...
git-tree-sha1 = "eb431cbd022bc55bf9cd1e21abe89b366270b403"
...
[[deps.PairPlots]]
deps = ["Contour", "Distributions", "KernelDensity", "Latexify", "LinearAlgebra", "Makie", "Measures", "NamedTupleTools", "OrderedCollections", "PolygonOps", "PrecompileTools", "Printf", "RecipesBase", "Requires", "StaticArrays", "Statistics", "StatsBase", "Tables"]
git-tree-sha1 = "c45fc647b95b760abbd2df52b0c90e6c79991e42"
